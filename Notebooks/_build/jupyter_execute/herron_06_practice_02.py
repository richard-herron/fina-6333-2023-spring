...
# # Herron Topic 6 - Practice (Wednesday 2:45 PM, Section 2)
...
crsp = (
    ...
    .assign(
        date=lambda x: x['date'].dt.to_period(freq='M'), # returns span a month, so I prefer to work with periods instead of dates
...
pd.qcut(x=np.arange(10), q=2, labels=False)
...
mom = (
    pd.concat(
        objs=[
            ret_1m, 
            ret_11m.shift(2),
            port_11m.shift(2), # base portfolio assignments on avail. info. and avoid short-term reversal
