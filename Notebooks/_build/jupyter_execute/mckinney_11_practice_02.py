...
returns = (
    ...
    .assign(
        ...
        Intraday = lambda x: (x['Close'] / x['Open']) - 1,
...
returns.add(1).prod().sub(1).mul(100)
...
returns_a = returns.resample('A', kind='period').agg(totret)


# We can use `.count()` to remove years with too few observations.

# In[11]:


counts_a = returns.resample('A', kind='period').count()


# In[12]:


returns_a.loc[counts_a['Total'] >= 250 ].mul(100).plot(kind='bar')
...
(returns_a['Overnight'] > returns_a['Intraday']).mean()
